...
module.exports = {
    ...
    vue: {
        createComponent: async (filePath, options) => {
            ...
            const targetFilePath = `${ROOT_DIR}/${workingDir}/${inputObj.path}/${inputObj.file}.vue`
            ...
            if (fs.existsSync(targetFilePath)) messages.vueComponentCreated(`${workingDir}/${inputObj.path}/${inputObj.file}.vue`)
        }
    },
    ...
    vueRouter : {
        createRouteModule: async (filePath, options) => {
            ...
            const inputObj =  getPathAsObj(filePath)
            // 4) final target directory
            const targetModuleFilePath = `${ROOT_DIR}/${workingDir}/modules/${inputObj.path}/${inputObj.file}.js`
            ...
            // 13) Confirm file is created
            if (fs.existsSync(targetModuleFilePath)) messages.routerModuleCreated(`${workingDir}/modules/${inputObj.path}/${inputObj.file}.js`)
