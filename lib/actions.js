...
module.exports = {
        ...
        init: async actionData => {
            ...
            if (!createNew && !createFromTemplate){
                if (!hasPackageJSON) {
                  ...
                  console.log('Directory already contains a package.json file. Aborted.')
                  return 
                }
            }
            ...
            if (hasConfig) {
                console.log(`directory already contains a ${APP_CONFIG_FILE} file`)
            } else if (!hasPackageJSON) {
                console.log(`directiry does not contain a ${APP_PACKAGE_FILE} file`)
            } else {
    ...
    vue: {
      createComponent: async actionData => {
    ...
    config: {
        rebuild: async () => {
            ...
            if (!hasPackageJSON) {
                console.log(`X <------- directiry does not contain a ${APP_PACKAGE_FILE} file (make message const)`)
                return
            }
            ...
            let envInfo = await getEnvInfo()
            let data = {
                appName: packageJSONParsed.name,
                templateURL: null,
                baseDir: path.basename(path.resolve(targetDir)),
                workingDir: targetDir,
                env: envInfo,
                hasConfig: hasConfig,
                hasPackageJSON: hasPackageJSON,
