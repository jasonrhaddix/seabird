...
module.exports = {
    ...
    vue: {
        createComponent: async (filePath, options) => {
            ...
            const workingDir = APP_PATH__COMPONENTS
    ...
    vueRouter : {
        createRouteModule: async (filePath, options) => {
            ...
            const workingDir = APP_PATH__ROUTER
            // 3) convert user input path to path object (path, file)
            const inputObj =  getPathAsObj(filePath)
            // 4) final target directory
            const targetModuleFilePath = `${rootDir}/${workingDir}/modules/${inputObj.path}/${inputObj.file}.js`
            ...
            let compiledStr = await ejs.renderFile(template, data)
            // 12) write file and path if path doesn't exist
            fsExtra.outputFileSync(targetModuleFilePath, compiledStr, err => {
                if (err) console.log(err)
            })

            // 13) Confirm file is created
            if (fs.existsSync(targetModuleFilePath)) console.log(`Router module created - ${inputObj.file}.js`)
            ...
            const importMatch = await indexFile.match(REGEX_PATTERN__IMPORT)
            let lastMatch = importMatch[importMatch.length-1]
            let importIdentifier = indexFile.replace(lastMatch, `${lastMatch}\n${APP_DELIMITER__SPLIT}`)

            const vueRouterMatch = await importIdentifier.match(REGEX_PATTERN__VUE_ROUTER)
            let vueRouterMatchLast = vueRouterMatch[vueRouterMatch.length-1]
            let strAddedSplitIdent = importIdentifier.replace(vueRouterMatchLast, `${vueRouterMatchLast}\n  ${APP_DELIMITER__SPLIT}`) */
                        
            // let splitString = strAddedSplitIdent.split(APP_DELIMITER__SPLIT)
            // let newstr = `${splitString[0]} \nimport ejs from ejs\n ${splitString[1]}`

            // console.log(strAddedSplitIdent)
