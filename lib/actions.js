...
module.exports = {
    ...
    vue: {
        createComponent: async (filePath, options) => {
            ...
            // 8) Removes user options with undefined props
            Object.keys(options).forEach(key => options[key] === undefined ? delete options[key] : {})
            // 9) Overwrites default options with user defined
            Object.assign(data, options)
            
            // *************************************
            // Compile and write template
            // *************************************
            // 10) render file with ejs
            let compiledStr = await ejs.renderFile(template, data)
            // 11) write file and path if path doesn't exist
            fsExtra.outputFileSync(targetFilePath, compiledStr, err => {
                if (err) console.log(err)
            })

            // 12) Confirm file is created
    ...
    vueRouter : {
        createRouteModule: async actionData => {
            ...
            let rootDir = await getRootPath()

            let routerDir = await config.read('services.vue-router.path')
            if (!routerDir) return
            
            let moduleData = getPathAsObj(actionData.moduleName)
            let modulePath = `${rootDir}/src/${routerDir}${moduleData.path}/${moduleData.file}.js`

            let options = {}
            let data = { 
                moduleName: moduleData.file,
                children: [
                    { name: 'child1' },
                    { name: 'chil2' }
                ]
            }
            let html = await ejs.renderFile(`${__dirname}/templates/Vue-router/module.ejs`, data, options)
            
            fsExtra.outputFileSync(modulePath, html, err => {
                if (err) console.log(err)
            })
        },
